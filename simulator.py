...
class Simulator:
    def __init__(self, rt : RaceTrack, steps_per_frame: int | None = None):
        self.rt = rt
        # Default: render every frame when not provided
        self.steps_per_frame = 1 if steps_per_frame is None else steps_per_frame
        self.headless = (steps_per_frame == 0)
    ...
    def run(self):
        try:
            ...
            if not self.lap_finished:
                steps = self.steps_per_frame if self.steps_per_frame > 0 else 1
                for _ in range(steps):
                    desired = controller(self.car.state, self.car.parameters, self.rt, self.simulation_time)
            ...
            # Fixed HUD in top-right corner (GUI only)
            if not self.headless:
            ...
            # Plot trajectory with speed colors
                if not self.headless and len(self.trajectory) > 1:
